...
try:
    ...
    print "Missing {0!s}, aborting ...".format(blockfile)
    sys.exit(1)
...
for line in blocks.readlines():
    ...
    try:
        ...
        print "Failed to process line: {0!s}".format((line))
        continue
    start = "0x" + start
    end = "0x" + end
    try:
...
blocks.close()
print "Parsed {0:d} blocks descriptions".format((len(BlockNames.keys())))
...
for block in blockAliases:
    ...
    for comp in alist:
        if BlockNames.has_key(comp):
            ...
            print "Alias {0!s}: {1!s} not in Blocks".format(alias[0], comp)
            continue
...
try:
    ...
    print "Missing {0!s}, aborting ...".format(catfile)
    sys.exit(1)
...
blocks.close()
print "Parsed {0:d} char generating {1:d} categories".format(nbchar, len(Categories.keys()))
...
for block in bkeys:
    ...
    if flag:
        ...
        flag = 1
    output.write('  {{"{0!s}", xmlUCSIs{1!s}}}'.format(block, name))
output.write('};\n\n')
...
for name in ckeys:
    if flag:
        ...
        flag = 1
    output.write('  {{"{0!s}", xmlUCSIsCat{1!s}}}'.format(name, name))
output.write('};\n\n')
...
for name in ckeys:
  if len(Categories[name]) > minTableSize:
    ...
    for range in ranges:
      ...
      if high < 0x10000:
        if numshort == 0:
          pline = "static const xmlChSRange xml{0!s}S[] = {{".format(name)
          sptr = "xml{0!s}S".format(name)
        else:
          pline += ", "
        numshort += 1
      else:
        if numlong == 0:
          if numshort > 0:
            output.write(pline + " };\n")
          pline = "static const xmlChLRange xml{0!s}L[] = {{".format(name)
          lptr = "xml{0!s}L".format(name)
        else:
          pline += ", "
        numlong += 1
      if len(pline) > 60:
        output.write(pline + "\n")
        pline = "    "
      pline += "{{{0!s}, {1!s}}}".format(hex(low), hex(high))
    output.write(pline + " }};\nstatic xmlChRangeGroup xml{0!s}G = {{{1!s},{2!s},{3!s},{4!s}}};\n\n".format(name, numshort, numlong, sptr, lptr))


output.write(
"""static xmlUnicodeNameTable xmlUnicodeBlockTbl = {{xmlUnicodeBlocks, {0!s}}};
static xmlUnicodeNameTable xmlUnicodeCatTbl = {{xmlUnicodeCats, {1!s}}};
...
""".format(len(BlockNames), len(Categories)) )

for block in bkeys:
    name = string.replace(block, '-', '')
    header.write("XMLPUBFUN int XMLCALL xmlUCSIs{0!s}\t(int code);\n".format(name))
    output.write("/**\n * xmlUCSIs{0!s}:\n * @code: UCS code point\n".format((name)))
    output.write(" *\n * Check whether the character is part of {0!s} UCS Block\n".format(
                 (block)))
    output.write(" *\n * Returns 1 if true 0 otherwise\n */\n");
    output.write("int\nxmlUCSIs{0!s}(int code) {{\n    return(".format(name))
    flag = 0
    for (start, end) in BlockNames[block]:
        if flag:
            ...
            flag = 1
        output.write("((code >= {0!s}) && (code <= {1!s}))".format(start, end))
    output.write(");\n}\n\n")
...
for name in ckeys:
    ranges = Categories[name]
    header.write("XMLPUBFUN int XMLCALL xmlUCSIsCat{0!s}\t(int code);\n".format(name))
    output.write("/**\n * xmlUCSIsCat{0!s}:\n * @code: UCS code point\n".format((name)))
    output.write(" *\n * Check whether the character is part of {0!s} UCS Category\n".format(
                 (name)))
    output.write(" *\n * Returns 1 if true 0 otherwise\n */\n");
    output.write("int\nxmlUCSIsCat{0!s}(int code) {{\n".format(name))
    if len(Categories[name]) > minTableSize:
        output.write("    return(xmlCharInRange((unsigned int)code, &xml{0!s}G)".format(name))
    else:
        start = 1
        for range in ranges:
            (begin, end) = range;
            if start:
                ...
                output.write(" ||\n           ");
            if (begin == end):
                output.write("(code == {0!s})".format((hex(begin))))
            else:
                output.write("((code >= {0!s}) && (code <= {1!s}))".format(
                         hex(begin), hex(end)))
    output.write(");\n}\n\n")
