...
class AnnotationCCodeWriter(CCodeWriter):
    ...
    def save_annotation(self, source_filename, target_filename):
        ...
        f.write(u'<!-- Generated by Cython {0!s} -->\n'.format(Version.watermark))
        f.write(u'<html>\n')
        f.write(u"""
<head>
...
function toggleDiv(id) {
    ...
    if (theDiv.style.display != 'block') theDiv.style.display = 'block';
        ...
        """)
        f.write(u'<body>\n')
        f.write(u'<p>Generated by Cython {0!s}\n'.format(Version.watermark))
        c_file = Utils.decode_filename(os.path.basename(target_filename))
        f.write(u'<p>Raw output: <a href="{0!s}">{1!s}</a>\n'.format(c_file, c_file))
        ...
        def annotate(match):
            ...
            calls[group_name] += 1
            return ur"<span class='{0!s}'>{1!s}</span>".format(
        ...
        for line in lines:
            ...
            color = u"FFFF{0:02x}".format(int(255/(1+score/10.0)))
            f.write(u"<pre class='line' style='background-color: #{0!s}' onclick='toggleDiv(\"line{1!s}\")'>".format(color, k))

            f.write(u" {0:d}: ".format(k))
            for c, cc, html in special_chars:
                line = line.replace(cc, html)
            f.write(line.rstrip())

            f.write(u'</pre>\n')
            f.write(u"<pre id='line{0!s}' class='code' style='background-color: #{1!s}'>{2!s}</pre>".format(k, color, code))
        f.write(u'</body></html>\n')
        f.close()
...
class AnnotationItem(object):
    ...
    def start(self):
        return u"<span class='tag {0!s}' title='{1!s}'>{2!s}".format(self.style, self.text, self.tag)
